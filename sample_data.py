...
2) hospital_equipment, exam_item 커버리지는 병원 홈페이지/제휴 조사를 통해
   실측 데이터로 교체하는 것을 권장합니다. (공공데이터에는 보유 장비 스펙까지는
   나오지 않는 경우가 많습니다)
"""
...
# 병원별 특성을 다양화하기 위한 등급 프로필 (장비/항목 커버리지, 가격대에 영향)
# tier가 높을수록 고가장비 보유 확률/항목 커버리지/가격이 높아짐
TIER_BY_INDEX = [3, 2, 1, 2, 2, 1, 3, 1, 1, 3, 2, 3, 1, 2, 2]

EQUIPMENT_BY_TIER = {
    1: ["CT", "도플러초음파", "골밀도측정기(DEXA)"],
    2: ["CT", "MRI", "도플러초음파", "골밀도측정기(DEXA)", "유방촬영기(맘모그래피)", "수면내시경 전용실"],
    3: ["CT", "MRI", "PET-CT", "도플러초음파", "골밀도측정기(DEXA)",
        "유방촬영기(맘모그래피)", "수면내시경 전용실", "관상동맥CT(CCTA)"],
}

CHECKUP_TYPES_BY_TIER = {
    1: [("일반검진(국가검진)", 0, 150000, 1.5), ("여성검진", 150000, 350000, 2.0),
        ("남성검진", 150000, 350000, 2.0)],
    2: [("종합검진", 400000, 900000, 3.5), ("일반검진(국가검진)", 0, 150000, 1.5),
        ("여성검진", 300000, 600000, 2.5), ("남성검진", 300000, 600000, 2.5),
        ("직장인 종합검진", 350000, 700000, 3.0)],
    3: [("종합검진", 800000, 2500000, 5.0), ("암검진", 500000, 1500000, 4.0),
        ("여성검진", 400000, 900000, 3.0), ("남성검진", 400000, 900000, 3.0),
        ("직장인 종합검진", 600000, 1200000, 4.0), ("영유아검진", 100000, 250000, 1.0)],
}
...
def populate_sample_data():
    ...
    for idx, (name, si, gu, addr, phone, est_year, certs) in enumerate(SAMPLE_HOSPITALS):
        ...
        # 장비 등록 (약간의 랜덤 변동을 줘서 완전히 tier 그대로 복사되지 않게)
        equip_list = EQUIPMENT_BY_TIER[tier].copy()
        if tier < 3 and random.random() < 0.25:
            # 하위 tier 병원도 가끔 MRI 정도는 추가 보유 (현실 반영)
        ...
        # 검사항목 커버리지 등록 (tier 비율만큼 랜덤 샘플링, 최신 장비 관련 항목은 장비 보유 여부와 연동)
        ...
        # 장비 연동 항목은 장비가 있어야만 제공 가능하도록 보정
        equipment_linked_items = {
            "뇌 MRI/MRA": "MRI",
            "전신 PET-CT": "PET-CT",
            "관상동맥CT": "관상동맥CT(CCTA)",
            "유방촬영술": "유방촬영기(맘모그래피)",
            "골밀도검사(DEXA)": "골밀도측정기(DEXA)",
        }
...
if __name__ == "__main__":
    ...
    print("샘플 데이터 삽입 완료:", get_connection())
